(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const l of t)if(l.type==="childList")for(const d of l.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&n(d)}).observe(document,{childList:!0,subtree:!0});function s(t){const l={};return t.integrity&&(l.integrity=t.integrity),t.referrerPolicy&&(l.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?l.credentials="include":t.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function n(t){if(t.ep)return;t.ep=!0;const l=s(t);fetch(t.href,l)}})();const Z=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:3001/api":"/api";let j=localStorage.getItem("token");async function g(a,e={}){const s={"Content-Type":"application/json",...e.headers};j&&(s.Authorization=`Bearer ${j}`);const n=await fetch(`${Z}${a}`,{...e,headers:s});if(!n.ok){const l=await n.json().catch(()=>({error:"Error de conexión"}));throw new Error(l.error||`HTTP ${n.status}`)}const t=n.headers.get("content-type");return t&&(t.includes("spreadsheet")||t.includes("csv")||t.includes("octet-stream"))?n:n.json()}const u={setToken(a){j=a},login:(a,e)=>g("/auth/login",{method:"POST",body:JSON.stringify({username:a,password:e})}),getMe:()=>g("/auth/me"),searchLeads:(a,e,s)=>g("/leads/search",{method:"POST",body:JSON.stringify({city:a,niche:e,limit:s})}),getLeads:(a={})=>{const e=new URLSearchParams(a).toString();return g(`/leads?${e}`)},saveLeads:a=>g("/leads/save",{method:"POST",body:JSON.stringify({leads:a})}),saveManualLead:a=>g("/leads/manual",{method:"POST",body:JSON.stringify(a)}),enrichLead:(a,e)=>g(`/leads/${a}/enrich`,{method:"PUT",body:JSON.stringify(e)}),updateLeadStatus:(a,e)=>g(`/leads/${a}/status`,{method:"PUT",body:JSON.stringify({status:e})}),deleteLead:a=>g(`/leads/${a}`,{method:"DELETE"}),exportCSV:async(a={})=>{const e=new URLSearchParams(a).toString(),n=await(await g(`/export/csv?${e}`)).blob(),t=URL.createObjectURL(n),l=document.createElement("a");l.href=t,l.download=`leads_${new Date().toISOString().split("T")[0]}.csv`,l.click(),URL.revokeObjectURL(t)},exportExcel:async(a={})=>{const e=new URLSearchParams(a).toString(),n=await(await g(`/export/excel?${e}`)).blob(),t=URL.createObjectURL(n),l=document.createElement("a");l.href=t,l.download=`leads_${new Date().toISOString().split("T")[0]}.xlsx`,l.click(),URL.revokeObjectURL(t)},getUsers:()=>g("/admin/users"),createUser:a=>g("/admin/users",{method:"POST",body:JSON.stringify(a)}),updateUser:(a,e)=>g(`/admin/users/${a}`,{method:"PUT",body:JSON.stringify(e)}),deleteUser:a=>g(`/admin/users/${a}`,{method:"DELETE"}),getStats:()=>g("/admin/stats")},V={success:"✅",error:"❌",info:"ℹ️",warning:"⚠️"},K=4e3;function X(){let a=document.getElementById("toast-container");return a||(a=document.createElement("div"),a.id="toast-container",a.className="toast-container",document.body.appendChild(a)),a}function o(a,e="info"){const s=X(),n=document.createElement("div");n.className=`toast toast-${e}`,n.innerHTML=`
  ...
  `;try{let i={totalLeads:0,weekLeads:0,searches:0,enriched:0};if(s){const r=await u.getStats();i.totalLeads=r.totalLeads||0,i.searches=r.totalSearches||0,i.enriched=r.enrichment_rate!=null?Math.round(r.enrichment_rate)+"%":r.enrichmentRate?Math.round(r.enrichmentRate)+"%":"0%",i.weekLeads=r.leads_this_week||r.weekLeads||0}else{const r=await u.getLeads(),E=Array.isArray(r)?r:r.leads||[];i.totalLeads=E.length;const H=new Date;H.setDate(H.getDate()-7),i.weekLeads=E.filter(U=>new Date(U.created_at)>H).length,i.enriched=E.filter(U=>U.enriched||U.is_enriched).length,i.searches="-"}document.getElementById("stats-grid").innerHTML=`
          ...
          <tbody>
            ${c.slice(0,5).map((r,E)=>`
              <tr class="animate-fade-in stagger-${E+1}">
                <td class="font-semibold">${M(r.name||r.business_name||"-")}</td>
                <td class="text-muted">${M(r.city||"-")}</td>
                <td class="text-muted">${M(r.niche||"-")}</td>
                <td>${r.phone?`<a href="tel:${r.phone}">${M(r.phone)}</a>`:'<span class="text-dim">-</span>'}</td>
                <td>${r.email?`<a href="mailto:${r.email}">${M(r.email)}</a>`:'<span class="text-dim">-</span>'}</td>
                <td>${te(r.status)}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
      `}catch(i){o("Error al cargar el dashboard: "+i.message,"error"),document.getElementById("stats-grid").innerHTML=`
      <div class="stat-card stat-card-gradient-1 animate-slide-up stagger-1">
        <span class="stat-icon">📊</span>
  ...
  `;const m=setInterval(()=>{i=Math.min(i+1,b.length-1);const c=document.getElementById("search-progress-text"),r=document.getElementById("search-progress-fill");c&&(c.textContent=b[i]),r&&(r.style.width=`${Math.min(10+(i+1)*15,90)}%`)},3e3);try{const c=await u.searchLeads(e,s,n);clearInterval(m);let r=Array.isArray(c)?c:c.leads||c.results||[];N&&N.user&&N.user.role!=="admin"&&(r=r.filter(E=>!E.already_saved)),k=r,w.clear(),l.innerHTML="",k.length===0?d.innerHTML=`
      ...
      `:ie(d),t.disabled=!1,t.innerHTML="🔍 Buscar Leads"}catch(c){clearInterval(m),l.innerHTML="",o("Error en la búsqueda: "+c.message,"error"),t.disabled=!1,t.innerHTML="🔍 Buscar Leads"}}function ie(a){a.innerHTML=`
  ...
  `,document.getElementById("select-all").addEventListener("change",e=>{const s=e.target.checked;k.forEach((n,t)=>{if(n.already_saved)return;const l=document.getElementById(`lead-cb-${t}`);l&&(l.checked=s),s?w.add(t):w.delete(t)}),Q()}),window.__saveSelectedLeads=async()=>{if(w.size===0){o("Selecciona al menos un lead para guardar","warning");return}const e=document.getElementById("save-selected-btn");e.disabled=!0,e.innerHTML='<span class="loading-spinner"></span> Guardando...';const s=Array.from(w).map(n=>k[n]);try{await u.saveLeads(s),o(`✅ ${s.length} leads guardados correctamente`,"success"),e.innerHTML="✅ Guardados",setTimeout(()=>{e.disabled=!1,e.innerHTML="💾 Guardar Seleccionados"},2e3)}catch(n){o("Error al guardar: "+n.message,"error"),e.disabled=!1,e.innerHTML="💾 Guardar Seleccionados"}}}function le(a,e){const s=C(a.name||a.business_name||"-"),n=C(a.address||"-"),t=a.phone||"",l=a.email||"",d=a.website||"",b=a.rating||a.stars||0,i=t||l,m=a.already_saved,c=a.saved_by_name;let r=s;return m&&(r+=` <span class="badge badge-danger" style="margin-left: 8px; font-size: 0.7rem; font-weight: normal; padding: 2px 6px;">🔴 Ocupado por ${C(c)}</span>`),`
  ...
  `,t.addEventListener("click",l=>{l.target===t&&t.remove()}),document.body.appendChild(t),t}function D(){const a=document.getElementById("modal-overlay");a&&a.remove()}const O=10;let L=1,y=[],I=[],_=null,p=null;async function oe(a,e){L=1,y=[],I=[],p=null,_=e,a.innerHTML=`
            ...
            <div class="flex flex-wrap gap-2">
              ${Object.entries(i).map(([c,r])=>`
                  <div class="chip" 
                    ...
                    👤 <strong>${r.name}</strong>: <span style="color: var(--color-success); font-weight: bold;">${r.count}</span>
                  </div>
                `).join("")}
            </div>
          </div>
  ...
  `)};window.__saveEnrichment=async a=>{const e=document.getElementById("save-enrich-btn");e.disabled=!0,e.innerHTML='<span class="loading-spinner"></span> Guardando...';const s=document.getElementById("enrich-phone-confirm").value.trim(),n=document.getElementById("enrich-needs").value.trim();if(!s){o("⚠️ La confirmación de teléfono es obligatoria","warning"),document.getElementById("enrich-phone-confirm").focus(),e.disabled=!1,e.innerHTML="💾 Guardar Enriquecimiento";return}if(!n){o("⚠️ El campo de necesidades es obligatorio","warning"),document.getElementById("enrich-needs").focus(),e.disabled=!1,e.innerHTML="💾 Guardar Enriquecimiento";return}const t={instagram:document.getElementById("enrich-instagram").value.trim(),facebook:document.getElementById("enrich-facebook").value.trim(),tiktok:document.getElementById("enrich-tiktok").value.trim(),linkedin:document.getElementById("enrich-linkedin").value.trim(),other_social:document.getElementById("enrich-other").value.trim(),phone_confirm:s,business_needs:n,weaknesses:document.getElementById("enrich-weaknesses").value.trim(),notes:document.getElementById("enrich-notes").value.trim()};try{await u.enrichLead(a,t);const l=y.find(d=>d.id===a);l&&(l.is_enriched=1,l.instagram=t.instagram,l.facebook=t.facebook,l.tiktok=t.tiktok,l.linkedin=t.linkedin,l.other_social=t.other_social,l.phone_confirm=t.phone_confirm,l.business_needs=t.business_needs,l.weaknesses=t.weaknesses,l.notes=t.notes),D(),z(),o("Lead enriquecido correctamente","success")}catch(l){o("Error al enriquecer: "+l.message,"error"),e.disabled=!1,e.innerHTML="💾 Guardar Enriquecimiento"}};async function de(){try{const a=document.getElementById("export-csv-btn");if(!a)return;a.disabled=!0,a.innerHTML='<span class="loading-spinner"></span> Exportando...';let e;if(p){const n=y.find(t=>(t.user_username||"").toLowerCase()===p);n&&(e=n.user_id)}const s={city:document.getElementById("filter-city").value.trim(),niche:document.getElementById("filter-niche").value.trim(),status:document.getElementById("filter-status").value,user_id:e};await u.exportCSV(s),o("CSV descargado correctamente","success"),a.disabled=!1,a.innerHTML="📥 Exportar CSV"}catch(a){o("Error al exportar CSV: "+a.message,"error");const e=document.getElementById("export-csv-btn");e&&(e.disabled=!1,e.innerHTML="📥 Exportar CSV")}}async function ce(){try{const a=document.getElementById("export-excel-btn");a.disabled=!0,a.innerHTML='<span class="loading-spinner"></span> Exportando...';let e;if(p){const n=y.find(t=>(t.user_username||"").toLowerCase()===p);n&&(e=n.user_id)}const s={city:document.getElementById("filter-city").value.trim(),niche:document.getElementById("filter-niche").value.trim(),status:document.getElementById("filter-status").value,user_id:e};await u.exportExcel(s),o("Excel descargado correctamente","success"),a.disabled=!1,a.innerHTML="📊 Exportar Excel"}catch(a){o("Error al exportar Excel: "+a.message,"error");const e=document.getElementById("export-excel-btn");e&&(e.disabled=!1,e.innerHTML="📊 Exportar Excel")}}function ue(a){const e={nuevo:{class:"info",label:"Nuevo"},contactado:{class:"warning",label:"Contactado"},convertido:{class:"success",label:"Convertido"},descartado:{class:"danger",label:"Descartado"}},s=e[a]||e.nuevo;return`<span class="badge badge-${s.class}">${s.label}</span>`}function h(a){const e=document.createElement("div");return e.textContent=a||"",e.innerHTML}window.__openCreateManualLead=()=>{P("➕ Agregar Lead Manual",`
  ...
  `;try{const e=await u.getUsers();T=Array.isArray(e)?e:e.users||[],pe(a)}catch(e){o("Error al cargar usuarios: "+e.message,"error"),a.innerHTML=`
      <div class="empty-state">
        <span class="empty-icon">⚠️</span>
  ...
  `),document.getElementById("edit-user-form").addEventListener("submit",async s=>{s.preventDefault();const n=document.getElementById("save-user-btn");n.disabled=!0,n.innerHTML='<span class="loading-spinner"></span> Guardando...';const t={username:document.getElementById("edit-username").value,full_name:document.getElementById("edit-fullname").value,email:document.getElementById("edit-email").value,role:document.getElementById("edit-role").value},l=document.getElementById("edit-password").value;l&&(t.password=l);try{await u.updateUser(a,t),D(),o("Usuario actualizado correctamente","success"),A()}catch(d){o("Error al actualizar: "+d.message,"error"),n.disabled=!1,n.innerHTML="💾 Guardar Cambios"}}))};window.__deleteUser=async a=>{const e=T.find(s=>s.id===a);if(confirm(`¿Estás seguro de eliminar al usuario "${(e==null?void 0:e.full_name)||(e==null?void 0:e.username)}"?`))try{await u.deleteUser(a),o("Usuario eliminado","success"),A()}catch(s){o("Error al eliminar: "+s.message,"error")}};async function be(){const a=document.getElementById("tab-stats");a.innerHTML=`
    <div class="loading-container">
      <div class="loading-spinner loading-spinner-lg"></div>
      <span class="loading-text">Cargando estadísticas...</span>
    </div>
  `;try{G=await u.getStats(),ge(a)}catch(e){o("Error al cargar estadísticas: "+e.message,"error"),a.innerHTML=`
      <div class="empty-state">
        <span class="empty-icon">📊</span>
  ...
  `;try{const e=await u.getLeads({all:!0});x=Array.isArray(e)?e:e.leads||[],F(a)}catch(e){o("Error al cargar leads: "+e.message,"error"),a.innerHTML=`
      <div class="empty-state">
        <span class="empty-icon">⚠️</span>
        <h4 class="empty-title">Error al cargar</h4>
        <p class="empty-description">${e.message}</p>
    ...
    `:`
      ...
      `:""}
    `}
  `}window.__adminGoToPage=a=>{$=a,F(document.getElementById("tab-all-leads"))};window.__adminDeleteLead=async a=>{if(confirm("¿Estás seguro de eliminar este lead y liberarlo para que otros asesores lo puedan captar?"))try{await u.deleteLead(a),x=x.filter(e=>e.id!==a),F(document.getElementById("tab-all-leads")),o("Lead eliminado y liberado correctamente","success")}catch(e){o("Error al eliminar el lead: "+e.message,"error")}};window.__adminExportCSV=async()=>{try{await u.exportCSV({all:!0}),o("CSV descargado","success")}catch(a){o("Error: "+a.message,"error")}};window.__adminExportExcel=async()=>{try{await u.exportExcel({all:!0}),o("Excel descargado","success")}catch(a){o("Error: "+a.message,"error")}};function he(a){const e={nuevo:{class:"info",label:"Nuevo"},contactado:{class:"warning",label:"Contactado"},convertido:{class:"success",label:"Convertido"},descartado:{class:"danger",label:"Descartado"}},s=e[a]||e.nuevo;return`<span class="badge badge-${s.class}">${s.label}</span>`}function f(a){const e=document.createElement("div");return e.textContent=a||"",e.innerHTML}function J(a,e){var t,l,d,b;const s=((t=e.user)==null?void 0:t.role)==="admin",n=window.location.hash||"#dashboard";a.innerHTML=`
